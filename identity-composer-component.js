...
function Ctrl($rootScope, $scope, brCredentialLibraryService) {
  ...
  model.consumerQuery = self.consumerQuery;
  ...
  scope.$watch(function() {return self.consumerQuery;}, init, true);
  scope.$watch(
  ...
  function init() {
    ...
    var credentials = jsonld.getValues(self.identity, 'credential').map(
      function(credential) {
